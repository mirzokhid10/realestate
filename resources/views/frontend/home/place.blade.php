                    ...
                    <div class="place-block-one">
                        <div class="inner-box">
                            <figure class="image-box"><img src="{{ asset($skip_state_1->state_image) }}" alt=""
                                    style="width:370px; height:275px;"></figure>
                            <div class="text">
                    ...
                    <div class="place-block-one">
                        <div class="inner-box">
                            <figure class="image-box"><img src="{{ asset($skip_state_3->state_image) }}" alt=""
                                    style="width:770px; height:275px;"></figure>
                            <div class="text">
...
@php
    $states = App\Models\State::take(4)->get(); // Fetch the first 4 states
    $properties = [];

    foreach ($states as $index => $state) {
        $properties[$index] = App\Models\Property::where('state', $state->id)->get();
    }
@endphp
    ...
    <div class="auto-container">
        ...
        <div class="sortable-masonry">
            <div class="items-container row clearfix">
                @foreach ($states as $index => $state)
                    <div
                        class="col-lg-{{ $index == 3 ? '8' : '4' }} col-md-6 col-sm-12 masonry-item small-column all illustration brand marketing software">
                        <div class="place-block-one">
                            <div class="inner-box">
                                <figure class="image-box">
                                    <img src="{{ asset($state->state_image) }}" alt=""
                                        style="width:{{ $index == 3 ? '770px' : '370px' }}; height:{{ $index == 3 ? '275px' : '580px' }};">
                                </figure>
                                <div class="text">
                                    <h4><a href="{{ route('state.details', $state->id) }}">{{ $state->state_name }}</a>
                                    </h4>
                                    <p>{{ count($properties[$index]) }} Properties</p>
                                </div>
                            </div>
                        </div>
                    </div>
